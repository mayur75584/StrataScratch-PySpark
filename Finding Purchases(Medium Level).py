...
df=df.withColumn("next_Day",lead("created_at",1).over(window_spec))
df=df.withColumn("Date_Difference",datediff(df["next_Day"],df["created_at"]))
df=df.filter(df.Date_Difference<7).select("user_id").distinct()
